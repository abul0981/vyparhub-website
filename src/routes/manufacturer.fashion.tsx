...
export const Route = createFileRoute("/manufacturer/fashion")({
  head: () => ({
    meta: [
      { title: "Fashion Manufacturer Application — VyparHub" },
      { name: "description", content: "Apply to list your fashion / apparel brand on VyparHub's B2B network." },
    ],
  }),
  component: FashionApply,
});
        ...
        <div className="rounded-3xl border border-border bg-card/95 p-7 shadow-2xl backdrop-blur">
          {step !== "done" && (
            <>
              ...
              <h1 className="mt-4 font-display text-2xl font-black">Fashion Manufacturer Application</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                Apna brand, dukan ya company VyparHub Fashion network par list karein.
              </p>
            </>
          )}
          ...
          {step === "details" && (
            ...
            >
              <input
                ...
                className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-orange"
              />
              <input
              ...
              <button disabled={saving} className="w-full rounded-xl bg-orange py-3 text-sm font-bold text-orange-foreground shadow-glow disabled:opacity-60">
          ...
          {step === "done" && (
            <div className="py-6 text-center">
              ...
              <h2 className="mt-4 font-display text-2xl font-black">Application Received</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                After verification, our team will connect with you shortly.
              </p>
              <button
                onClick={() => navigate({ to: "/" })}
                className="mt-6 rounded-xl bg-orange px-5 py-3 text-sm font-bold text-orange-foreground shadow-glow"
              >
                {t("goHome")}
              </button>
            </div>
